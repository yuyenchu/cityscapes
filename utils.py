...
def get_eefm_cross_attention(CLASS_NUM = 3):
    ...
    x4a = layers.Conv2D(160, 1, padding='same', activation='sigmoid')(x4a)
    x4 = layers.Multiply()([x4a, x4])
    p5 = layers.Add(name='fuse1')([p5, x4])

    p4 = layers.Conv2DTranspose(80,3,2,padding='same',name='up4')(p5)
    x3 = layers.Concatenate()([x3, x3p])
    x3a = layers.Attention(use_scale=True)([p4, x3, p4])
    x3a = layers.Conv2D(80, 1, padding='same', activation='sigmoid')(x3a)
    ...
    x2a = layers.Conv2D(40, 1, padding='same', activation='sigmoid')(x2a)
